...
**CMake + a C++ compiler are mandatory** for the client: `wxdragon` (wxWidgets) and `opus`/`audiopus_sys` build their C/C++ from source on first build (slow, then cached). If `cmake` is missing, `pip install --user cmake` provides a usable binary (`~/.local/bin/cmake`); put it on `PATH`. Linux also needs `libgtk-3-dev` + `libasound2-dev`. `client/.cargo/config.toml` sets `CMAKE_POLICY_VERSION_MINIMUM=3.5` so CMake 4.x accepts the vendored Opus build — don't remove it.
