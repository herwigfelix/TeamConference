...
rem Laufzeit per Name geladen und muessen daher als Datei daneben liegen. Fehlen sie,
rem laeuft die App trotzdem (tts faellt auf die Windows-Sprachausgabe WinRT zurueck)
rem — nur ohne NVDA-Routing.
rem
rem Quelle sind die fest ins Repo gelegten DLLs unter packaging\windows. Frueher
rem wurden sie aus target\release kopiert (dort legt tolk-sys sie beim Build ab),
rem aber das ist unzuverlaessig: laeuft tolk-sys' build.rs wegen Cargo-Caching
rem (rust-cache in der CI) nicht erneut, fehlen die losen DLLs im target-Ordner.
set "DLLSRC=%SCRIPT_DIR%packaging\windows"
if exist "%DLLSRC%\nvdaControllerClient64.dll" copy /y "%DLLSRC%\nvdaControllerClient64.dll" "%OUT%\" >nul
if exist "%DLLSRC%\SAAPI64.dll" copy /y "%DLLSRC%\SAAPI64.dll" "%OUT%\" >nul
