...
use crate::db::queries::{self, DbRoom};
...
impl RoomManager {
    ...
    /// `room_list` an jeden Nutzer des Tenants.
    pub async fn broadcast_room_list(&self, tenant: &str) {
        let list = self.get_room_list(tenant).await.unwrap_or_default();
        self.users
            .broadcast_tenant(tenant, Message::new("room_list", serde_json::json!({ "rooms": list })))
            .await;
    }

    /// Raum nur innerhalb des eigenen Tenants auflösen (S3).
